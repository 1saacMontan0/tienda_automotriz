...
require ("../../models/lecturas/compras_excel.php"); # tabla clientes
require ("../../models/lecturas/compras_json.php"); # tabla clientes
...
$redirec = "../../pages/gestion/compras.php";  
...
switch ($_POST['reporte']) {
    case 'pdf':
        ...
        $html .= "<h2 class='titulo' style='font-size:18pt;'>Reporte de Compras</h2>";
        ...
        $html .= '</thead>';
        $html .= '<tbody>';
        $html .= generar_compras($conexion, $_SESSION['id_empresa']);
        $html .= '</tbody>';
        $html .= '</table>';
    ...
    case 'excel':
        ...
        $encabezados = [
            'Tipo', 'Nombre', 'Descripción', 'Compatible Desde', 'Compatible Hasta',
            'Unidades', 'Precio Compra', 'Precio Venta', 'Marca', 'Modelo', 'Registro'
        ];
        ...
        compras_excel($conexion, $_SESSION['id_empresa'], $sheet);
        ...
        header('Content-Disposition: attachment; filename="reporte-inventario-servicios.xlsx"');
    ...
    case 'json':
        ...
        // 2. Obtener los datos
        $datos = obtener_datos_inventario_servicios_json($conexion, $_SESSION['id_empresa']);

        // 4. HEADERS PARA FORZAR DESCARGA
        header('Content-Type: application/json; charset=utf-8');
        // Fuerza al navegador a descargar el archivo con un nombre específico
        header('Content-Disposition: attachment; filename="reporte_inventario_servicios.json"');
