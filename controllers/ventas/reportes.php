...
# genera reportes pdf, excel y json de ventas.
...
require ("../../models/lecturas/ventas_pdf.php"); # tabla clientes
require ("../../models/lecturas/ventas_excel.php"); # tabla clientes
require ("../../models/lecturas/ventas_json.php"); # tabla clientes
...
# Discrimina y genera el reporte esperado.
switch ($_POST['reporte']) {
    case 'pdf':
        ...
        $html .= "<h2 class='titulo' style='font-size:18pt;'>Reporte de Ventas</h2>";
        ...
        $html .= '</thead>';
        $html .= '<tbody>';
        $html .= generar_ventas($conexion, $_SESSION['id_empresa']);
        $html .= '</tbody>';
        $html .= '</table>';
        ...
        $mpdf->Output('Reporte_ventas.pdf', \Mpdf\Output\Destination::DOWNLOAD);
        break;
    case 'excel':
        ...
        $encabezados = [
            'Cliente', 'Producto', 'Marca', 'Modelo', 'Compatible Desde', 'Compatible Hasta', 'Unidades',
            'Total', 'Ganancia', 'Tipo de factura', 'Fecha'
        ];
        ...
        ventas_excel($conexion, $_SESSION['id_empresa'], $sheet);
    ...
    case 'json':
        ...
        // 2. Obtener los datos
        $datos = ventas_json($conexion, $_SESSION['id_empresa']);

        // 4. HEADERS PARA FORZAR DESCARGA
        header('Content-Type: application/json; charset=utf-8');
        // Fuerza al navegador a descargar el archivo con un nombre específico
        header('Content-Disposition: attachment; filename="reporte_ventas.json"');
